...
export function SidebarNav({ items, variant, title, subtitle }: SidebarNavProps) {
  ...
  return (
    <>
      {/* ---------- MOBILE TOP BAR ---------- */}
      <div className="fixed inset-x-0 top-0 z-40 flex h-16 items-center gap-3 border-b border-border/60 bg-background/80 px-4 backdrop-blur-xl lg:hidden">
      ...
      <aside
        className={cn(
          'fixed inset-y-0 left-0 z-40 hidden flex-col border-r border-border/60 bg-card/50 backdrop-blur-xl transition-[width] duration-300 lg:flex',
