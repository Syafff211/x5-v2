...
export function LoginForm({ role }: LoginFormProps) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    toast.success(isAdmin ? 'Selamat datang, Admin!' : 'Login berhasil. Selamat belajar!')
  ...
  return (
    <>
      <div className="mb-7 text-center">
        ...
        >
          {isAdmin ? <Shield className="h-7 w-7" /> : <GraduationCap className="h-7 w-7" />}
        </div>
        <h1 className="text-2xl font-bold tracking-tight">{isAdmin ? 'Login Admin' : 'Login Siswa'}</h1>
        <p className="mt-1.5 text-sm text-muted-foreground">
          {isAdmin ? 'Panel pengelolaan kelas X-5' : 'Masuk ke dashboard kelas X-5'}
        </p>
      </div>
        ...
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              autoComplete="email"
              required
              placeholder={isAdmin ? 'admin@x5-sman1.web.id' : 'nama@student.sman1purbalingga.sch.id'}
              value={email}
