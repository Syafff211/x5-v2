...
const QUICK_LINKS = [
  ...
  { label: 'Lihat Tugas', href: '/dashboard/assignments', icon: ClipboardList, accent: 'from-indigo-500 to-violet-500' },
  { label: 'Unduh Materi', href: '/dashboard/materials', icon: BookOpen, accent: 'from-sky-500 to-blue-500' },
  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div className="relative">
          ...
          <p className="mt-2 max-w-xl text-sm text-white/85 sm:text-base">
            {pending.length > 0
              ? `Kamu punya ${pending.length} tugas yang belum dikumpulkan. Yuk kerjakan sekarang!`
              : 'Semua tugas sudah dikumpulkan. Kerja bagus, pertahankan ya!'}
          </p>
          <div className="mt-5 flex flex-wrap gap-2">
            ...
            <Button asChild size="sm" variant="ghost" className="border border-white/30 text-white hover:bg-white/15">
              <Link href="/dashboard/assignments">
                Lihat Tugas <ArrowRight className="h-4 w-4" />
              </Link>
            </Button>
          </div>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard label="Tugas Aktif" value={activeAssignments.length} icon={ClipboardList} accent="indigo" trend={`${pending.length} belum dikumpulkan`} delay={0.05} />
        ...
        <Card glass className="lg:col-span-2">
          <CardHeader className="flex-row items-center justify-between space-y-0">
            <CardTitle className="flex items-center gap-2 text-base">
              <ClipboardList className="h-4 w-4 text-primary" /> Tugas Mendatang
            </CardTitle>
          ...
          <CardContent className="space-y-2.5">
            {upcoming.length === 0 && <p className="py-6 text-center text-sm text-muted-foreground">Tidak ada tugas aktif. 🎉</p>}
            {upcoming.map((a) => {
