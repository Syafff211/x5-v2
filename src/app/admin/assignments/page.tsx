...
export default function AdminAssignmentsPage() {
  ...
  function save(e: React.FormEvent) {
    e.preventDefault()
    if (form.title.trim().length < 4) return toast.error('Judul tugas minimal 4 karakter.')
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Kelola Tugas"
        description={`${assignments.length} tugas · ${submissions.filter((s) => s.score == null).length} menunggu penilaian`}
        action={<Button variant="gradient" onClick={openAdd}><Plus className="h-4 w-4" /> Buat Tugas</Button>}
      />
      ...
      {list.length === 0 ? (
        <EmptyState
          ...
          description="Buat tugas pertama untuk kelas X-5."
          action={<Button variant="gradient" onClick={openAdd}><Plus className="h-4 w-4" /> Buat Tugas</Button>}
        />
      ) : (
              ...
              <Card key={a.id} glass className="flex h-full flex-col p-5">
                <div className="mb-3 flex items-start justify-between gap-3">
                  ...
                  <div className="flex shrink-0 gap-1">
                    <Button variant="ghost" size="icon-sm" onClick={() => openEdit(a)} aria-label="Edit tugas"><Pencil className="h-4 w-4" /></Button>
                    <Button variant="ghost" size="icon-sm" onClick={() => setConfirmDelete(a)} aria-label="Hapus tugas" className="text-destructive hover:bg-destructive/10">
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editing ? 'Edit Tugas' : 'Buat Tugas Baru'}</DialogTitle>
            <DialogDescription>Isi detail tugas yang akan dibagikan ke siswa kelas X-5.</DialogDescription>
          </DialogHeader>
          <form onSubmit={save} className="space-y-4">
            ...
            <DialogFooter>
              ...
              <Button type="submit" variant="gradient">{editing ? 'Simpan Perubahan' : 'Buat Tugas'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
